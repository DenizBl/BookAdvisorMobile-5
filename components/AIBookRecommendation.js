...
const getRandomColor = (index) => {
   // Her kitap için index'e göre renk seç, karışıklık için Math.random da ekle
   const colorIndex =
      (index + Math.floor(Math.random() * cardColors.length)) % cardColors.length;
...
const AIBookRecommendation = () => {
   ...
   const handleSubmit = async () => {
      ...
      try {
         ...
         if (result && result.length > 0) {
            ...
            const shuffledResults = result.sort(() => 0.5 - Math.random()).slice(0, 15);

            setBooks(shuffledResults);
         } else {
            setError(
               'Bu kriterle ilgili kitap bulunamadı. Lütfen başka bir duygu durumu deneyin.'
            );
         }
      } catch (err) {
         console.error('Hata:', err);
         setError(
            'Üzgünüz, şu anda kitap önerileri getirilemiyor. Lütfen daha sonra tekrar deneyin.'
         );
      } finally {
         setLoading(false);
      }
   ...
   const MoodButton = ({ title }) => (
      <TouchableOpacity
         ...
         onPress={() =>
            setMood((prev) =>
               prev ? prev + ', ' + title.toLowerCase() : title.toLowerCase()
            )
         }
      >
         ...
         <View style={styles.formContainer}>
            ...
            <View style={styles.moodButtonsContainer}>
               <ScrollView
                  ...
                  contentContainerStyle={styles.moodButtonsScroll}
               >
            ...
            <View style={styles.resultsContainer}>
               ...
               {books
                  ...
                  .map((book, index) => {
                     ...
                     const cardColor = getRandomColor(index);
                     return (
                        <View
                           key={book.id}
